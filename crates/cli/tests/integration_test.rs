  ...
  match output {
    ...
    Err(e) => {
      if e.kind() == std::io::ErrorKind::NotFound {
        eprintln!(
                    "Binary not found. Please build the project first with: cargo build -p hyuqueue-topic-rss-cli (binary name: hyuqueue-topic-rss)"
                );
      }
...
fn test_version_flag() {
  ...
  match output {
    Ok(output) => {
      ...
      let stdout = String::from_utf8_lossy(&output.stdout);
      assert!(
        stdout.contains("hyuqueue-topic-rss"),
        "Expected version text to contain 'hyuqueue-topic-rss', got: {}",
        stdout
      );
    }
    Err(e) => {
