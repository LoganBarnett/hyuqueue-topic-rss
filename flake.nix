{
  description = "Hyuqueue topic for RSS feeds over JSON-RPC stdio";
  inputs = {
  ...
  outputs = {
    ...
    perSystem = forAllSystems (system: let
      ...
      crates = {
        # CRATE:cli:begin
        cli = {
          name = "hyuqueue-topic-rss-cli";
          binary = "hyuqueue-topic-rss";
          description = "Hyuqueue topic process for RSS feeds (JSON-RPC over stdio)";
        };
        # CRATE:cli:end
        # CRATE_ENTRIES
      ...
      packages =
        ...
        // {
          default =
            craneLib.buildPackage (commonArgs // {pname = "hyuqueue-topic-rss";});
        };
    in {
      inherit packages;
